...
//later on this should be switched to async or mio (harder to debug, so do when working)
fn main() {
    //this is just temp configs of rustls
    ...
    let num_threads: &u8 = &settings[0]
        .parse()
        .expect("Conversiion error on reading yaml to variable.");
    ...
    let listener2 = TcpListener::bind(https_socket).unwrap(); //this is the listener for port 2 (ie https)
...
fn return_file(in_file: String) -> String {
    ...
    if is_in_vector(in_file.clone(), dirs.clone()) == true {
        //here we are checking if the user enters a valid file, i.e. index.html or favicon.ico. .clone() is used as variables need reusing later
        in_file //returning it straigh back
    } else {
        //this is where we deal with files that should have .html appended to them
        full_file.push_str(extension); //appending .html to the file

        if is_in_vector(full_file.to_string(), dirs) == true {
            //if the .html appended file exists, it returns it
            full_file.to_string()
        } else {
            //not all .html appended files exist, so just returns the 404.html page
            println!("404.html");
            "404.html".to_string()
        }
    }
...
struct Config {
    //this is an index from the yaml configuration file. New entries need to be added in order when features are added
    num_threads: u16,
    target: String,
    http_port: String,
    https_port: String,
